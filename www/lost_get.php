...
$fetch="SELECT * FROM lost ORDER BY date DESC  ";
...
$com="SELECT * FROM lost_comment WHERE lost_id='$lost_id' ";
...
//close comment
  ...
  ?>
  <form class="ui reply form <?php  echo"$lost_id"?>">
    <div class="field">
     <input type="text" name="last-name" placeholder="Reply Here" class="cmnt_box <?php  echo"$lost_id" ?>" >
    </div>
    <div class="ui blue labeled submit icon button <?php  echo"$lost_id" ?>"  u_id="<?php echo $live_id;?>" lost_id="<?php  echo"$lost_id"?>"  >
...
<div class="ui feed" >
 ...
 <?php  
  ...
  <form class="ui reply form <?php echo"$lost_id"?>">
    ...
    <div class="ui blue labeled submit icon button <?php echo"$lost_id"?>"   lost_id="<?php  echo"$lost_id"?>"  >
      <i class="icon edit"></i> Add Reply
    </div>
  </form>
  </div><!-- ui comments -->
...
if(cmnt!="")
{
	   ...
	   $.ajax({
			...
			beforeSend: function() {
        // setting a timeout
      $('.ui.blue.labeled.submit.icon.button.'+lost_id).addClass("loading");
    },
            data: 'cmnt='+cmnt+'&lost_id='+lost_id+'&u_id='+u_id,
            url:"lost_comment.php",
            success: function (data) {
        $('.ui.blue.labeled.submit.icon.button.'+lost_id).removeClass("loading");
	   ...
	   $.ajax({
        ...
        $(this).addClass('loading');
            ...
            success: function (data) {
			$('.ui.reply.form.'+lost_id).before(data);
